...
### KB-010（P2，已修复）：PDF 页码、Excel 工作表 metadata 丢失，且声明支持的 `.xls` 实际不可读
...
### KB-011（P2，已修复）：模型可用性检查忽略 `is_selected`
...
### KB-012（P2，已修复）：多个重操作仍在 UI 事件循环同步执行
...
### KB-013（P2，已修复）：数据库、文件和向量后端之间没有原子提交协议
...
### KB-015（P2，已修复）：知识库内容直接进入提示词，缺少注入边界处理
...
### KB-017（P2，已修复）：多知识库的 reranker 选择依赖无序集合
...
### KB-018（P3，已修复）：列表和文档查看器在大规模下会制造过多 UI 控件/内存复制
...
### KB-019（P3，已修复）：文件系统异常与数据库状态处理不对称
...
### KB-020（P3，已修复）：表名截断和旧后端资源缺少生命周期管理
...
因此，完成第一、二阶段后，知识库的失败恢复和检索语义关键路径已具备回归保护；第三阶段的格式语义、性能、输入验证和可信性工作也已完成并纳入回归测试，可按发布检查结果决定是否发布。

## 11. 第三阶段实施记录（2026-08-11）

### 11.1 完成范围

本次完成 KB-010 至 KB-020，覆盖文件格式语义、UI 响应性、输入校验、提示词边界、向量索引生命周期和大规模文档查看。此前第一、二阶段修复保持不变。

### 11.2 实施过程

1. 解析层新增 `ParsedDocumentSegment`。PDF loader 的 `page`、XLSX loader 的 `sheet`/`row_count` 作为分段 metadata 传入 chunk；编排器逐段切分并重新分配全局 chunk index。移除 `.xls` 映射和文件选择器声明，避免把不兼容格式伪装成 XLSX。
2. 将上传前的文件类型、大小、哈希、复制和目录创建移入 worker thread；将切分、sqlite-vec/SeekDB 写入和文档删除移出 UI 事件循环。混合检索缓存 active index 的 `ChunkData` 与 BM25 语料，最多保留 16 个索引缓存，避免每条消息重建 BM25。
3. 新建 `kb_validation` 并在 UI、服务持久化边界同时执行：`chunk_size > 0`、`0 <= overlap < chunk_size`、两个 top-k 均为正整数、threshold 为有限 `[0, 1]` 数字；模型可用性额外要求 router URL/API key 完整且对应 `RouterModel.is_selected`。
4. 重排器忽略负数、越界、重复、非整数和非有限分数；若结果中没有有效索引则失败关闭。RAG 上下文将来源与正文作 HTML 转义，使用 `<reference>` 结构，并明确资料是不可信的用户上传内容、不能执行其中指令。
5. 索引物理资源引入 migration v9：`active_vector_table_name`、`active_vector_backend_fingerprint` 和 cleanup job 的目标表/后端指纹。新表名使用完整 KB UUID；迁移保留旧八位前缀表地址。跨后端的清理任务不会自动删除，`get_orphaned_vector_resources()` 提供只读审计清单，避免误删其他后端资源。
6. 文档列表改为数据库分页（单页 100 条且不读取 `content_text`）；查看器使用 `substr()` 分片追加而不重复复制前缀。剪贴板复制限制为 1,000,000 字符，超限时要求导出；导出按 64 KiB 流式写入。KB 创建/删除和文档上传/删除增加文件系统失败的补偿或显式失败处理，删除文档的文件删除失败会尝试重建含原文档的活动索引。
